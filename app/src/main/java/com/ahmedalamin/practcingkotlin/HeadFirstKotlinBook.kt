...
*Add conditional branches to your code using if and else. The
else clause is optional.
*You can use if as an expression so that it returns a value. In this
case, the else clause is mandatory.
...
*Create an array using the arrayOf function: var myArray =
arrayOf(1, 2, 3)
...
*A function can have parameters, so you can pass more than one
value to it.
...
*A function can return a value. You must define the type of value (if
any) it returns.
*A Unit return type means that the function doesn’t return anything.
Choose for loops over while loops when you know how many
times you want to repeat the loop code.
*The readLine() function reads a line of input from the standard
*input stream. It returns a String value, the text entered by the user.
